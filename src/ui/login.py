...
class LoginView:
    ...
    def _initialize(self):
        ...
        ttk.Label(self._frame, text="Kirjautuminen").grid(row=0, column=0, columnspan=2, sticky=constants.W, padx=5, pady=5)

        ttk.Label(self._frame, text="Käyttäjätunnus").grid(row=1, column=0, sticky=constants.W, padx=5, pady=5)
        ...
        ttk.Button(self._frame, text="Kirjaudu", command=self._login).grid(row=3, column=0, sticky=(constants.E, constants.W), padx=5, pady=5)
        ttk.Button(self._frame, text="Rekisteröidy", command=self._handle_show_register).grid(row=3, column=1, sticky=(constants.E, constants.W), padx=5, pady=5)
    ...
    def _login(self):
        try:
            self._user_service.login(self._username.get(), self._password.get())
            self._message.set("")
